...
public class PhotoManagementAdapter extends BaseAdapter {
    ...
    public void loadImagesExtras(){
        dataForm = context.getSharedPreferences("M16PhotoManagement", Context.MODE_PRIVATE);
    ...
    public void loadImagesBurnerAssembly(){
        ...
        for (String s : namesDataForm) {
            dataForm = context.getSharedPreferences(s, Context.MODE_PRIVATE);
            boolean addPhoto = dataForm.getBoolean("addPhotoBoolean", false);
            if (addPhoto) {
                loadPath(dataForm);
            }
